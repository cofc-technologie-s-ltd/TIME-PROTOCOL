...
import unittest
import time
from time_protocol import Node, Wallet, P2PNode, Block
...
class TestFullIntegration(unittest.TestCase):
    ...
    def test_wallet_transaction_flow(self):
        ...
        # Mine 2 blocks to Alice (100 TIME total)
        node.mine_pending_transactions(alice.address, [])
        node.mine_pending_transactions(alice.address, [])
        alice_balance_before = alice.get_balance(node.ledger)
        self.assertEqual(alice_balance_before, 100.0)

        # Alice sends 10 TIME to Bob (no fee for simplicity)
