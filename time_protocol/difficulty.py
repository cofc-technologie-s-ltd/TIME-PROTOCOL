...
class DifficultyManager:
    ...
    - Retarget every N blocks
    - Target: TARGET_BLOCK_TIME seconds per block
    ...
    def calculate_next_difficulty(self, chain: List) -> int:
        if len(chain) < 2:
            return chain[0].difficulty if chain else self.MIN_DIFFICULTY
    ...
    def get_retarget_info(self, chain: List) -> dict:
        if len(chain) < 2:
            return {
                "current_difficulty": chain[0].difficulty if chain else 1,
                "next_difficulty": chain[0].difficulty if chain else 1,
        ...
        blocks_until = self.retarget_interval - blocks_since if blocks_since != 0 else self.retarget_interval
