...
class PeerDiscovery:
    ...
    def __init__(self, own_port: int, bootstrap_nodes: Optional[List[Tuple[str, int]]] = None):
        """
        Create a PeerDiscovery instance.
        
        bootstrap_nodes:
        - None (default) → use DEFAULT_BOOTSTRAP_NODES
        - []              → no bootstrap nodes (empty list is intentional)
        - [list of tuples] → use that specific list
        """
        self.own_port = own_port
        self.known_peers: dict = {}
        
        # FIX: distinguish between None (use default) and [] (intentional empty)
        ...
        for host, port in self.bootstrap:
            if port != own_port or host not in ("127.0.0.1", "localhost"):
